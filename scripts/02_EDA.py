...
# %%
data["yield_t_ha"].describe().to_frame("UK wheat yield (t/ha)")
...
by_decade = data.groupby("decade")["yield_t_ha"].agg(["mean", "std", "min", "max"])
by_decade.round(3)
...
for col, color in zip(temp_cols, ["#1f77b4", "#2ca02c", "#ff7f0e", "#d62728"]):
    axes[0].plot(
        data["year"],
        data[col],
...
for col, color in zip(rain_cols, ["#1f77b4", "#2ca02c", "#ff7f0e", "#d62728"]):
    axes[1].plot(
        data["year"],
        data[col],
        label=col.replace("_", " ").title(),
...
# %%
corr_with_yield.to_frame("corr_with_yield").round(3)
