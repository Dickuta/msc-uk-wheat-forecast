...
|---|---|---|---|
| FR-1 | Acquire raw public data reproducibly, with provenance (URL, checksum, date) | `stages/01` download + `manifest.csv` (`01_Data_Acquisition.py:163-183`) | enforced |
...
| FR-6 | Uncertainty: PI coverage vs nominal | §5.7 of `stages/05_Model.py`; verified in §5.10 | enforced |
| FR-7 | Bound the ceiling: oracle (perfect weather foresight) | §5.9 of `stages/05_Model.py`, `oracle_fc` factories | enforced |
| FR-8 | Self-verify against canonical expected values, fail loudly | `verify()` (`05_Model.py:613-689`) | enforced |
| FR-9 | Practitioner decision guide, traceable to the numbers | §5.11 of `stages/05_Model.py` → `data/outputs/decision_guide.md` | enforced |
...
(`stages/05_Model.py:656`). These catch structural leakage but do not undo
